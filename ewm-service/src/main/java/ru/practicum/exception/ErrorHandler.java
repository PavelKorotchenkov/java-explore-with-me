...
public class ErrorHandler {
	...
	@ExceptionHandler(CategoryUniqueNameViolationException.class)
	@ResponseStatus(HttpStatus.CONFLICT)
	public ApiError handleException(final CategoryUniqueNameViolationException e) {
		log.warn("Exception: {}", e.getMessage());
		String reason = "Category with that name already exists";
		HttpStatus status = HttpStatus.CONFLICT;
		return createApiError(e, reason, status);
	}
	...
	@ResponseStatus(HttpStatus.CONFLICT)
	public ApiError handleException(final UserUniqueEmailViolationException e) {
		log.warn("Exception: {}", e.getMessage());
